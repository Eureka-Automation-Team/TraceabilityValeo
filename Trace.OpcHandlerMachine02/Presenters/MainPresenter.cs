...
namespace Trace.OpcHandlerMachine02.Presenters
{
    public class MainPresenter
    {
        ...
        private bool LoggingData(IEnumerable<ItemValueResult> r, MachineModel machine, IEnumerable<PlcTagModel> machineTags)
        {
            ...
            foreach (var item in r)
            {
                ...
                if (!invalid)
                {
                    if (item.ItemName == _view.tagMainBlock + "ST2Code")
                    {
                        ...
                        /*Get Model Running from Station 1
            ...
            foreach (var item in r.Where(x => tagsPart.Any(s => s.Tag == x.ItemName)).OrderBy(o => o.ItemName))
            {
                ...
                if (!invalid)
                {
                    ...
                    if (item.ItemName == _view.tagMainBlock + "ST2PartSerialNo[6]")
                    {
                        part.LineNumber = 4;
                        part.PartName = "Hollow Screw";
                        part.SerialNumber = item.Value.ToString();
                    }
                    if (item.ItemName == _view.tagMainBlock + "ST2PartSerialNo[7]")
                    {
                        part.LineNumber = 5;
                        part.PartName = "Ejot screw M6X30";
                        part.SerialNumber = item.Value.ToString();
                    }
        ...
        private void Initailization(object sender, EventArgs e)
        {
            ...
            {
            new OPCVar("RequestVerify", "ST2ReqChkCodeVerify", OPCVarType.BOOL),
            new OPCVar("MachineStatus","ST2StatusMc", OPCVarType.SINT),
            new OPCVar("RequestLogging", "ST2ReqLogging", OPCVarType.BOOL),
            new OPCVar("ClockSystem", "ClockSystem", OPCVarType.BOOL),
            new OPCVar("TraceabilityRdy", "TraceabilityRdy", OPCVarType.BOOL),
            new OPCVar("ST2LoggingApp", "ST2LoggingApp", OPCVarType.SINT),
